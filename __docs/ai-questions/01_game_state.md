...
| Fog of war | `Player::getFieldDirectVisible`, which relies on the field, `std::vector<std::vector<VisionFieldInfo>> m_FogVisionFields` so this contains the info the player can see. Also see another function like `Unit:isStealthed` which makes use ofthese same functions to determine what units we can see at what positions. You can see an example of how it is called from `ACTION_FIRE.js` where it is called from `getStepData()` |
